...
$('ul.tabs li').click(function(){var $this=$(this);var $theTab=$(this).attr('id');console.log($theTab);if($this.hasClass('active')){}else{$this.closest('.tabs_wrapper').find('ul.tabs li, .tabs_container .tab_content').removeClass('active');
$('.tabs_container .tab_content[data-tab="'+$theTab+'"], ul.tabs li[id="'+$theTab+'"]').addClass('active');}});});

function filterProducts() {
    // var dateFilter = $('#date-filter').val();
    var nameFilter = $('#name-filter').val(); // Lấy giá trị được chọn từ select
  
    // Ứng dụng bộ lọc
    $('table tbody tr').hide();
  
    $('table tbody tr').each(function() {
    //   var rowDate = $(this).find('td:nth-child(6)').text().trim();
      var rowName = $(this).find('td:nth-child(2)').text().trim();
  
      var showRow = true;
  
      if (
        (nameFilter && nameFilter !== 'all' && rowName.toLowerCase() !== nameFilter.toLowerCase()) // So sánh giá trị từ select với tên hàng
      ) {
        showRow = false;
      }
        ...
        listItem.innerHTML = `
            <div class="productviews">
                ...
                <div class="productviewscontent">
                    ...
                    <a href="javascript:void(0);" class="hideset" data-index="${i}" onclick="removeImage(this)">x</a>
                </div>
            </div>
        `;
...
function removeImage(element) {
    const indexToRemove = parseInt(element.getAttribute("data-index"));
...
function updateInputFiles() {
    ...
    // Create a new FileList and populate it with selectedFiles
